<template>
  <div>
    <div class="header">
      <span class="search-header">搜索用户数</span>
      <svg
        t="1649652942314"
        class="icon"
        viewBox="0 0 1024 1024"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        p-id="3425"
        width="16"
        height="16"
      >
        <path
          d="M512 0C229.23 0 0 229.23 0 512s229.23 512 512 512 512-229.23 512-512S794.77 0 512 0zM512 928c-229.75 0-416-186.25-416-416S282.25 96 512 96s416 186.25 416 416S741.75 928 512 928z"
          p-id="3426"
          fill="#8a8a8a"
        />
        <path
          d="M537.64 343.452c47.074 0 83.266-37.528 83.266-78.072 0-32.46-20.832-60.878-62.496-60.878-54.816 0-82.178 44.618-82.178 77.11C475.144 320.132 498.152 343.452 537.64 343.452z"
          p-id="3427"
          fill="#8a8a8a"
        />
        <path
          d="M533.162 728.934c-7.648 0-10.914-10.136-3.264-39.55l43.25-166.406c16.386-60.848 10.944-100.398-21.92-100.398-39.456 0-131.458 39.83-211.458 107.798l16.416 27.392c25.246-17.256 67.906-34.762 77.792-34.762 7.648 0 6.56 10.168 0 35.508l-37.746 158.292c-23.008 89.266 1.088 109.538 33.984 109.538 32.864 0 117.808-30.47 195.57-109.632l-18.656-25.34C575.354 716.714 543.05 728.934 533.162 728.934z"
          p-id="3428"
          fill="#8a8a8a"
        />
      </svg>
    </div>
    <div class="main">
      <span class="main-title">123132</span>
      <span class="main-content">17.1</span>
      <svg
        t="1649653690175"
        class="icon"
        viewBox="0 0 1024 1024"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        p-id="6390"
        id="mx_n_1649653690176"
        width="16"
        height="16"
      >
        <path
          d="M439.1 602.5c0 91.2-74 165.2-165.2 165.2-91.3 0-165.2-74-165.2-165.2 0-91.3 74-165.2 165.2-165.2 91.2 0 165.2 74 165.2 165.2"
          fill="#FFFFFF"
          p-id="6391"
        />
        <path
          d="M273.9 776c-95.6 0-173.5-77.8-173.5-173.5 0-95.6 77.8-173.5 173.5-173.5 95.6 0 173.5 77.8 173.5 173.5-0.1 95.7-77.9 173.5-173.5 173.5z m0-330.4c-86.6 0-157 70.4-157 157s70.4 157 157 157 157-70.4 157-157-70.5-157-157-157z"
          fill="#27272D"
          p-id="6392"
        />
        <path
          d="M411.3 602.5c0 75.9-61.5 137.4-137.4 137.4s-137.4-61.5-137.4-137.4S198 465.1 273.9 465.1s137.4 61.5 137.4 137.4"
          fill="#FFFFFF"
          p-id="6393"
        />
        <path
          d="M273.9 748.2c-80.3 0-145.7-65.3-145.7-145.7 0-80.3 65.3-145.7 145.7-145.7s145.7 65.3 145.7 145.7c0 80.3-65.4 145.7-145.7 145.7z m0-274.8c-71.2 0-129.2 57.9-129.2 129.2 0 71.2 57.9 129.2 129.2 129.2s129.2-57.9 129.2-129.2c0-71.3-58-129.2-129.2-129.2z"
          fill="#27272D"
          p-id="6394"
        />
        <path
          d="M694.1 709.1v27.3c0 41.2 12.1 79.1 25.5 110.1 0.2 0.5 1.3 1 1.5 1.4 20.8 45.1 52.6 73.7 87.6 73.7 17.5 0 34.3-7.1 49.1-20 0.3-0.2 1.8-3.6 2.1-3.9 37.8-33.4 62.7-91.5 62.7-188.7H694.1z"
          fill="#50BCFF"
          p-id="6395"
        />
        <path
          d="M718.4 736.3V375.1h112L599.2 105.5 368 375.1h101.4v359.8c0 40.9 17.5 78.6 41.6 109.4 0.3 0.5 0.2 0.9 0.7 1.4 36.7 46.1 92.3 75.9 155.5 75.9h144c-87.1-24.3-86.7-76.7-92.8-185.3"
          fill="#1A6DFF"
          p-id="6396"
        />
        <path
          d="M811.1 929.8h-144c-62.9 0-121.8-28.7-161.8-78.8-0.9-1-1.4-2-1.7-2.8-27.7-35.6-42.5-75-42.5-113.4V383.3H368c-3.2 0-6.1-1.9-7.5-4.8-1.3-2.9-0.9-6.4 1.2-8.8l231.2-269.6c3.1-3.6 9.4-3.6 12.5 0l231.2 269.6c2.1 2.4 2.6 5.9 1.2 8.8-1.3 2.9-4.3 4.8-7.5 4.8H726.7v352.8l0.6 11.7c5.4 98.9 7.9 144 86 165.8 4 1.1 6.5 5 6 9.1s-4 7.1-8.2 7.1z m-292.4-88.6c36.8 45.8 90.9 72.1 148.5 72.1H769c-50.8-29.8-53.6-81.9-58.2-164.6l-0.7-11.9V375.1c0-4.6 3.7-8.2 8.2-8.2h94L599.2 118.2 386 366.8h83.5c4.6 0 8.2 3.7 8.2 8.2v359.8c0 44.1 21.7 81 39.8 104.3 0.4 0.6 0.8 1.3 1.2 2.1zM325.4 527c2.8-2.8 2.8-7.4 0-10.2-2.8-2.8-7.4-2.8-10.2 0l-41.4 41.4-41.4-41.4c-2.8-2.8-7.4-2.8-10.2 0-2.8 2.8-2.8 7.4 0 10.2l40.9 40.9h-46.3c-4 0-7.2 3.2-7.2 7.2s3.2 7.2 7.2 7.2h49.8v21.3c0 4 3.2 7.2 7.2 7.2s7.2-3.2 7.2-7.2v-28.5c0-1.1-0.3-2.1-0.7-3l45.1-45.1z"
          fill="#282828"
          p-id="6397"
        />
        <path
          d="M284.6 575.1c0 4 3.2 7.2 7.2 7.2H331c4 0 7.2-3.2 7.2-7.2s-3.2-7.2-7.2-7.2h-39.2c-4-0.1-7.2 3.2-7.2 7.2zM331 615.3h-39.2c-4 0-7.2 3.2-7.2 7.2s3.2 7.2 7.2 7.2H331c4 0 7.2-3.2 7.2-7.2s-3.2-7.2-7.2-7.2zM273.8 615.3h-57c-4 0-7.2 3.2-7.2 7.2s3.2 7.2 7.2 7.2h49.8v55.6c0 4 3.2 7.2 7.2 7.2s7.2-3.2 7.2-7.2v-62.8c0-4-3.2-7.2-7.2-7.2z"
          fill="#282828"
          p-id="6398"
        />
        <path
          d="M621.3 690.6c0 99.6-80.8 180.4-180.4 180.4-99.7 0-180.4-80.8-180.4-180.4 0-99.6 80.8-180.4 180.4-180.4 99.6 0 180.4 80.8 180.4 180.4"
          fill="#FFFFFF"
          p-id="6399"
        />
        <path
          d="M440.9 879.2c-104 0-188.7-84.6-188.7-188.6S336.8 502 440.9 502c104 0 188.6 84.6 188.6 188.6s-84.6 188.6-188.6 188.6z m0-360.8c-94.9 0-172.2 77.2-172.2 172.1 0 94.9 77.2 172.2 172.2 172.2 94.9 0 172.1-77.2 172.1-172.2 0-94.8-77.2-172.1-172.1-172.1z"
          fill="#27272D"
          p-id="6400"
        />
        <path
          d="M590.9 690.6c0 82.9-67.2 150-150 150-82.9 0-150.1-67.2-150.1-150 0-82.9 67.2-150 150.1-150 82.8-0.1 150 67.1 150 150"
          fill="#FFFFFF"
          p-id="6401"
        />
        <path
          d="M440.9 848.9c-87.3 0-158.3-71-158.3-158.3 0-87.3 71-158.3 158.3-158.3 87.3 0 158.3 71 158.3 158.3 0 87.3-71 158.3-158.3 158.3z m0-300.1c-78.2 0-141.8 63.6-141.8 141.8 0 78.2 63.6 141.8 141.8 141.8 78.2 0 141.8-63.6 141.8-141.8 0-78.2-63.6-141.8-141.8-141.8z"
          fill="#27272D"
          p-id="6402"
        />
        <path
          d="M439 658.5c-2.1 0-4.2-0.8-5.8-2.4-3.2-3.2-3.2-8.4 0-11.7l53.2-53.2c3.2-3.2 8.4-3.2 11.7 0s3.2 8.4 0 11.7l-53.2 53.2c-1.7 1.6-3.8 2.4-5.9 2.4z"
          fill="#282828"
          p-id="6403"
        />
        <path
          d="M439 666.2h-65.3c-4.6 0-8.2-3.7-8.2-8.2s3.7-8.2 8.2-8.2H439c4.6 0 8.2 3.7 8.2 8.2s-3.6 8.2-8.2 8.2zM512.7 666.2h-44.9c-4.6 0-8.2-3.7-8.2-8.2s3.7-8.2 8.2-8.2h44.9c4.6 0 8.2 3.7 8.2 8.2s-3.6 8.2-8.2 8.2zM512.7 720.5h-44.9c-4.6 0-8.2-3.7-8.2-8.2 0-4.6 3.7-8.2 8.2-8.2h44.9c4.6 0 8.2 3.7 8.2 8.2 0.1 4.5-3.6 8.2-8.2 8.2z"
          fill="#282828"
          p-id="6404"
        />
        <path
          d="M439 698.8c-4.6 0-8.2-3.7-8.2-8.2V658c0-4.6 3.7-8.2 8.2-8.2 4.6 0 8.2 3.7 8.2 8.2v32.6c0.1 4.5-3.6 8.2-8.2 8.2zM439 720.5h-65.3c-4.6 0-8.2-3.7-8.2-8.2 0-4.6 3.7-8.2 8.2-8.2H439c4.6 0 8.2 3.7 8.2 8.2 0.1 4.5-3.6 8.2-8.2 8.2z"
          fill="#282828"
          p-id="6405"
        />
        <path
          d="M439 792.3c-4.6 0-8.2-3.7-8.2-8.2v-71.9c0-4.6 3.7-8.2 8.2-8.2 4.6 0 8.2 3.7 8.2 8.2v71.9c0.1 4.5-3.6 8.2-8.2 8.2zM439 658.5c-2.1 0-4.2-0.8-5.8-2.4L380 602.9c-3.2-3.2-3.2-8.4 0-11.7s8.4-3.2 11.7 0l53.2 53.2c3.2 3.2 3.2 8.4 0 11.7-1.7 1.6-3.8 2.4-5.9 2.4z"
          fill="#282828"
          p-id="6406"
        />
      </svg>
      <svg
        t="1649653769515"
        class="icon"
        viewBox="0 0 1024 1024"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
        p-id="7367"
        width="16"
        height="16"
      >
        <path
          d="M252.484267 566.749867h11.776v16.6912h-11.776zM815.684267 566.8352h10.461866v16.9984h-10.461866z"
          fill="#996E28"
          p-id="7368"
        />
        <path
          d="M825.361067 580.642133a6.826667 6.826667 0 0 0-6.980267-3.618133l-158.0032 21.418667L680.2432 201.386667a6.826667 6.826667 0 0 0 0-1.058134v-1.109333a6.826667 6.826667 0 0 0-6.826667-7.168H405.1968a6.826667 6.826667 0 0 0-6.826667 7.168v1.109333a6.826667 6.826667 0 0 0 0 1.058134l19.848534 397.038933-158.0032-21.4016a6.826667 6.826667 0 0 0-5.922134 11.4176l280.0128 301.277867a6.826667 6.826667 0 0 0 10.001067 0L824.32 588.424533a6.826667 6.826667 0 0 0 1.041067-7.7824z"
          fill="#996E28"
          p-id="7369"
        />
        <path
          d="M339.780267 523.7248H103.6288a17.066667 17.066667 0 1 1 0-34.133333h236.151467a13.380267 13.380267 0 1 0 0-26.760534h-33.467734a44.782933 44.782933 0 1 1 0.631467-89.565866h238.250667a17.066667 17.066667 0 0 1 0 34.133333h-238.250667a11.093333 11.093333 0 0 0-11.229867 9.5744 10.717867 10.717867 0 0 0 10.5984 11.7248h33.467734a47.5136 47.5136 0 1 1 0 95.0272z"
          fill="#D5382E"
          p-id="7370"
        />
        <path
          d="M545.194667 373.282133a17.066667 17.066667 0 0 1 0 34.133334h-238.250667a11.093333 11.093333 0 0 0-11.229867 9.5744 10.717867 10.717867 0 0 0 10.5984 11.7248h33.467734a47.5136 47.5136 0 1 1 0 95.0272H103.6288a17.066667 17.066667 0 1 1 0-34.133334h236.151467a13.380267 13.380267 0 1 0 0-26.760533h-33.467734a44.782933 44.782933 0 1 1 0.631467-89.565867h238.250667m0-10.24h-238.250667a55.022933 55.022933 0 1 0-0.631467 110.045867h33.467734a3.140267 3.140267 0 1 1 0 6.280533H103.6288a27.306667 27.306667 0 1 0 0 54.613334h236.151467a57.7536 57.7536 0 1 0 0-115.5072h-33.467734a0.512 0.512 0 0 1-0.290133-0.170667 0.3584 0.3584 0 0 1-0.119467-0.221867 1.416533 1.416533 0 0 1 1.041067-0.426666h238.250667a27.306667 27.306667 0 0 0 0-54.613334z"
          fill="#E8D4AB"
          p-id="7371"
        />
        <path
          d="M654.523733 591.854933h-1.5872l20.48-409.6H405.1968l20.48 409.6"
          fill="#AF3131"
          p-id="7372"
        />
        <path
          d="M654.523733 593.5616h-1.5872l20.48-409.6H405.1968l20.48 409.6"
          fill="#D5382E"
          p-id="7373"
        />
        <path
          d="M654.523733 600.388267h-1.5872a6.826667 6.826667 0 0 1-6.826666-7.168l20.1216-402.432H412.3648l20.1216 402.432-13.653333 0.682666-20.48-409.6a6.826667 6.826667 0 0 1 6.826666-7.168h268.253867a6.826667 6.826667 0 0 1 6.826667 7.168l-20.48 409.6-5.239467-0.256z"
          fill="#E8D4AB"
          p-id="7374"
        />
        <path
          d="M425.6768 589.704533l-166.365867-22.528L539.306667 868.471467l279.995733-301.294934-164.778667 22.528"
          fill="#D5382E"
          p-id="7375"
        />
        <path
          d="M425.6768 591.4112l-166.365867-22.528L539.306667 870.178133l279.995733-301.294933-164.778667 22.528"
          fill="#D5382E"
          p-id="7376"
        />
        <path
          d="M539.306667 874.837333a6.826667 6.826667 0 0 1-5.000534-2.184533L254.293333 571.357867a6.826667 6.826667 0 0 1 5.922134-11.4176l158.122666 21.418666-19.968-399.223466a6.826667 6.826667 0 0 1 6.826667-7.168h268.2368a6.826667 6.826667 0 0 1 6.826667 7.168l-19.968 399.240533 158.122666-21.435733a6.826667 6.826667 0 0 1 5.922134 11.4176L544.3072 872.6528a6.826667 6.826667 0 0 1-5.000533 2.184533zM277.2992 576.034133L539.306667 857.975467l262.007466-281.941334-147.3536 19.968a6.826667 6.826667 0 0 1-7.7312-7.099733l20.0192-400.2816H412.3648l20.0192 400.264533a6.826667 6.826667 0 0 1-7.7312 7.099734z"
          fill="#E8D4AB"
          p-id="7377"
        />
        <path
          d="M780.7488 226.338133h105.540267a17.066667 17.066667 0 0 1 0 34.133334h-105.540267a13.380267 13.380267 0 1 0 0 26.760533h8.0384a44.782933 44.782933 0 1 1 0.631467 89.565867H503.7056a17.066667 17.066667 0 0 1 0-34.133334h285.730133a10.717867 10.717867 0 0 0 10.5984-11.7248 11.076267 11.076267 0 0 0-11.229866-9.5744h-8.0384a47.5136 47.5136 0 1 1 0-95.0272z"
          fill="#D5382E"
          p-id="7378"
        />
        <path
          d="M886.289067 226.338133a17.066667 17.066667 0 0 1 0 34.133334h-105.540267a13.380267 13.380267 0 1 0 0 26.760533h8.0384a44.782933 44.782933 0 1 1 0.631467 89.565867H503.7056a17.066667 17.066667 0 0 1 0-34.133334h285.730133a10.717867 10.717867 0 0 0 10.5984-11.7248 11.076267 11.076267 0 0 0-11.229866-9.5744h-8.0384a47.5136 47.5136 0 1 1 0-95.0272h105.540266m0-10.24h-105.557333a57.7536 57.7536 0 1 0 0 115.5072h8.0384a1.416533 1.416533 0 0 1 1.041067 0.426667 0.341333 0.341333 0 0 1-0.119467 0.221867 0.802133 0.802133 0 0 1-0.221867 0.170666H503.7056a27.306667 27.306667 0 0 0 0 54.613334h285.730133a55.022933 55.022933 0 1 0-0.631466-110.045867h-8.0384a3.140267 3.140267 0 1 1 0-6.280533h105.540266a27.306667 27.306667 0 0 0 0-54.613334z"
          fill="#E8D4AB"
          p-id="7379"
        />
      </svg>
    </div>

    <div class="footer">
      <div class="charts" ref="charts"></div>
    </div>
  </div>
</template>

<script>
const echarts = require("echarts");
export default {
  name: 'lineCharts',
  data() {
    return {
      
    }
  },
  mounted() {
    // 初始化echarts实例
    let lineCharts = echarts.init(this.$refs.charts);
    // 配置数据
    lineCharts.setOption({
      xAxis: {
        show: false,
        type: 'category'
      },
      yAxis: {
        show: false
      },
      // 系列
      series: [
        {
          type: 'line',
          data: [10, 7, 33, 12, 88, 9],
          // 拐点样式
          itemStyle: {
            opacity: 0
          },
          lineStyle: {
            color: 'blue'
          },
          areaStyle: {
            color: {
              type: 'linear',
              x: 0,
              y: 0,
              x2: 0,
              y2: 1,
              colorStops: [{
                offset: 0, color: 'skyblue' // 0% 处的颜色
              }, {
                offset: 1, color: '#FFF' // 100% 处的颜色
              }],
              global: false // 缺省为 false
            }
          }
        }
      ],
      tooltip: {
        show: true
      },
      //布局调试
      grid: {
        left: 0,
        top: 0,
        right: 0,
        bottom: 0
      }
    })
  },
  methods: {
    
  }
}
</script>

<style scoped>
.header {
  display: flex;
  align-items: center;
  color: rgb(170, 168, 168);
}
.search-header {
  margin-right: 10px;
}
.main {
  display: flex;
  align-items: center;
  margin-top: 10px;
}
.main-title {
  margin-right: 30px;
}
.main-content {
  margin-right: 10px;
}
.charts {
  height: 100px;
  width: 100%;
}
</style>